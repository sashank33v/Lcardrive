...
export default function PortalProfilePage() {
  ...
  const inputClass = "w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#FACC15] bg-white transition-colors"
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="bg-white rounded-2xl p-5 space-y-4 border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input className={inputClass} value={form.first_name || ''} onChange={e => update('first_name', e.target.value)} placeholder="Sarah" />
          </div>
          <div>
            <label className={labelClass}>Last Name</label>
            <input className={inputClass} value={form.last_name || ''} onChange={e => update('last_name', e.target.value)} placeholder="Mitchell" />
          </div>
        </div>
        <div>
          <label className={labelClass}>Phone</label>
          <input className={inputClass} value={form.phone || ''} onChange={e => update('phone', e.target.value)} placeholder="04XX XXX XXX" type="tel" />
        </div>
        <div>
          <label className={labelClass}>Email</label>
          <input className={inputClass} value={form.email || ''} onChange={e => update('email', e.target.value)} placeholder="you@email.com" type="email" />
        </div>
        <div>
          <label className={labelClass}>Years Experience</label>
          <input type="number" className={inputClass} value={form.years_experience || ''} onChange={e => update('years_experience', parseInt(e.target.value))} min={0} max={50} placeholder="5" />
        </div>
      </div>

      {/* Bio */}
      <div className="bg-white rounded-2xl p-5 space-y-3 border border-gray-100" id="bio">
        <div className="flex items-center justify-between">
          <h2 className="font-bold text-gray-900">Bio</h2>
          <button
            onClick={generateBio}
            disabled={bioLoading}
            className="flex items-center gap-1.5 text-xs bg-yellow-50 text-yellow-700 border border-yellow-200 px-3 py-1.5 rounded-xl font-medium hover:bg-yellow-100 transition-colors disabled:opacity-50"
          >
          ...
          {[
            ...
            <div key={key} className="flex items-center justify-between">
              ...
              <button onClick={() => update(key, !form[key])}
                className={`w-11 h-6 rounded-full transition-colors relative ${form[key] ? 'bg-[#FACC15]' : 'bg-gray-200'}`}
              >
      ...
      <div className="bg-white rounded-2xl p-5 space-y-4 border border-gray-100">
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => update('dual_controls', !form.dual_controls)}
            className={`w-11 h-6 rounded-full transition-colors relative ${form.dual_controls ? 'bg-[#FACC15]' : 'bg-gray-200'}`}
          >
      ...
      <button onClick={save} disabled={saving}
        className="w-full bg-[#FACC15] text-gray-900 font-semibold py-4 rounded-2xl hover:bg-yellow-400 disabled:opacity-50 transition-colors text-base"
      >
