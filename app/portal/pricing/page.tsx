...
export default function PortalPricingPage() {
  ...
  const inputClass = "w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#FACC15] bg-white"
        ...
        {[
          ...
          return (
            <div key={key}>
              ...
              <div className="relative">
                <span className="absolute left-4 top-3 text-gray-500 text-sm">$</span>
                <input type="number" className={`${inputClass} pl-8`} value={pkg.price || ''} onChange={e => updatePkg(e.target.value)} placeholder={`e.g. ${((form.hourly_rate || 70) * hours * 0.9).toFixed(0)}`} />
              </div>
              {form.hourly_rate && (
