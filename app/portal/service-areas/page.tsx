...
export default function ServiceAreasPage() {
  ...
  const toggleTestCentre = (id: string) => {
    ...
    setForm((p: any) => ({
      ...p,
      familiar_test_centres: current.includes(id) ? current.filter((c: string) => c !== id) : [...current, id]
    }))
  }
      ...
      <div className="bg-white rounded-2xl p-5 space-y-4">
        ...
        <div className="relative">
          <input
            ...
            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#1A3CFF]"
          />
          {suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-lg z-10 mt-1">
              {suggestions.map(s => (
                <button key={s} onClick={() => addSuburb(s)} className="w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50 flex items-center gap-2">
                  <Plus size={14} className="text-[#1A3CFF]" /> {s}
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex flex-wrap gap-2">
          {(form.service_suburbs || []).map((s: string) => (
            <span key={s} className="flex items-center gap-1.5 bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1.5 rounded-full text-sm font-medium">
              {s}
      ...
      <div className="bg-white rounded-2xl p-5 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {(testCentres as any[]).map(tc => {
            ...
            return (
              <button key={tc.id} onClick={() => toggleTestCentre(tc.id)} className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium border-2 text-left transition-colors ${active ? 'bg-[#1A3CFF] text-white border-[#1A3CFF]' : 'bg-white text-gray-700 border-gray-200 hover:border-[#1A3CFF]'}`}>
                {active ? '✓' : '○'} {tc.name}
              </button>
            )
          })}
