...
export function AdminButton({ variant = 'nav' }: Props) {
  ...
  const handleClick = () => localStorage.setItem('lcardrive_admin_mode', 'true')
  ...
  if (variant === 'mobile-menu') {
    return (
      <Link href="/admin" onClick={handleClick}
        className="flex items-center gap-2.5 px-3 py-3 rounded-xl text-sm font-bold text-yellow-700 bg-yellow-50 hover:bg-yellow-100 transition-colors border border-yellow-200"
      >
  ...
  return (
    <Link href="/admin" onClick={handleClick}>
      <button className="flex items-center gap-1.5 bg-[#1A2444] text-yellow-400 text-xs font-bold px-3 py-2 rounded-xl hover:bg-[#1A2444]/90 active:scale-95 transition-all border border-yellow-500/30">
        <ShieldCheck size={13} />
        Admin
      </button>
    </Link>
